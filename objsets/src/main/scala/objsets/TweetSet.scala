...
abstract class TweetSet {
  ...
  def mostRetweeted: Tweet = 
    ...
    else mostRetweetedInc(this.head);
  
  def mostRetweetedInc(curr: Tweet): Tweet
   ...
   * and be implemented in the subclasses?
   */
  def descendingByRetweet: TweetList = descendingByRetweetAcc(Nil, this); 
  
  def descendingByRetweetAcc(t:TweetList, list:TweetSet):TweetList
...
class Empty extends TweetSet {
  ...
  def mostRetweetedInc(curr: Tweet): Tweet = curr
  def descendingByRetweetAcc(t:TweetList, list:TweetSet):TweetList = t
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedInc(curr: Tweet): Tweet = 
    if(this.head.retweets > curr.retweets) this.tail.mostRetweetedInc(this.head)
    else this.tail.mostRetweetedInc(curr)
    
  def descendingByRetweetAcc(t:TweetList, list:TweetSet):TweetList = 
    list.remove(list.mostRetweeted).descendingByRetweetAcc(t.prepend(list.mostRetweeted), list.remove(list.mostRetweeted)) 
...
object Nil extends TweetList {
  ...
  def prepend(t:Tweet): TweetList = new Cons(t, this)
  def isEmpty = true
}

class Cons(val head: Tweet, val tail: TweetList) extends TweetList {
  def isEmpty = false
  def prepend(t:Tweet): TweetList = new Cons(this.head, this.tail.prepend(t))
}
